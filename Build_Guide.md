...
## Final Added Features (April 2026)
...
- **Cycle Visualization (`screens/Cycle.js`)**: Replaced plain cards with a complete SVG circular ring using `react-native-svg`. The ring displays a progress bar mapped to the vulnerability score and color-codes the current phase according to the style guide (sage, rose, alert).
- **Journal Reliability (`screens/Journal.js`)**: Added keyboard padding for inputs and replaced numerical mood scores with expressive emoji faces.
- **Firebase Initialization (`utils/firebase.js`)**: Added initialization guards to prevent crashes during hot-reloads and tab switching.
- **Clinician Dashboard Auth**: Included `backend/scripts/seedClinician.js` to reliably create the clinician user role inside Firestore and Auth to successfully test the clinician dashboard.

---

## 22. UI Style Guide & Design System

### Design Philosophy
- **Quiet Luxury**: Feels like a premium wellness brand, not a hospital app. Soft textures, refined typography, generous whitespace.
- **Warm Safety**: Colors are warm, never harsh. Every screen feels like a safe space. Rose and sage over cold blues and grays.
- **Cultural Pride**: Tamil script is displayed beautifully and equally. Never as an afterthought.

### Color Palette (Hex Codes)
- **Nirantara Rose**: `#C97B84` (Primary brand color. CTA buttons, highlights, active states)
- **Rose Light**: `#F2D9DC` (Backgrounds, cards, soft highlights)
- **Calm Lavender**: `#9B8EC4` (AI companion, chat UI, secondary actions)
- **Healing Sage**: `#7BA68A` (Low risk, positive mood, success states)
- **Warm Cream**: `#FBF7F2` (App background. Never pure white)
- **Deep Charcoal**: `#2C2826` (All body text, headings, icons)
- **Alert Terracotta**: `#E8634A` (High risk alerts, crisis warnings ONLY)

**Risk Level Colors:**
- **Low Risk (Safe)**: Sage Light `#D6EAD9` background, Healing Sage `#7BA68A` icon/text
- **Moderate (Watch)**: `#FDF3E3` background, `#F0A830` icon, `#8B5E1A` text
- **High Risk (Alert)**: `#FDEAE6` background, Alert Terracotta `#E8634A` icon, `#9B2A15` text
- **Crisis (Urgent)**: Deep Charcoal `#2C2826` background, Alert Terracotta `#E8634A` icon, Warm White `#FEFCFA` text

### Typography System
- **Display Heading**: `Cormorant Garamond` 300/400 (Screen headers, Splash)
- **Body Text / Section Headings**: `DM Sans` 400/500
- **Tamil Text**: `Noto Sans Tamil` 400/500

### Non-Negotiable UI Rules
- **NEVER** use pure white (`#FFFFFF`) as a background.
- **NEVER** use black text on colored backgrounds.
- **NEVER** use color alone to show risk level (always pair color + icon + text).
- **NEVER** use sharp corners (0px radius).
- **NEVER** show Tamil text smaller than English text.
- **ALWAYS** use `#FBF7F2` warm cream as base background.
- **ALWAYS** use minimum 16px tap targets on mobile.
- **ALWAYS** add `accessibilityLabel` to every button.

### React Native Theme Constants
Copy this exactly into your `src/theme/theme.js` file:
...
## 23. Persona Architecture
**Single config — zero rebuild for new personas**

| Persona | Config Status | Features & Toggles |
| --- | --- | --- |
| 👩 **Women** | Active (Building Now) | ✓ Cycle LSTM active<br>✓ Hormonal risk boost<br>✓ All 8 triggers<br>✓ 44px tap targets<br>✓ Crisis + JITAI NLP |
| 👴 **Elderly** | Config Ready | – Cycle LSTM off<br>✓ Family view on<br>✓ 72px tap targets<br>✓ Voice-first mode<br>✓ Crisis + JITAI NLP |
| ♿ **Disabled** | Config Ready | – Cycle LSTM off<br>✓ Voice-only input<br>✓ 64px tap targets<br>✓ Screen reader full<br>✓ Crisis + JITAI NLP |
| 🧑 **General Adult** | Config Ready | – Cycle LSTM off<br>– No hormonal boost<br>✓ 7 triggers (no T1)<br>✓ Standard UI<br>✓ Crisis + JITAI NLP |
...
## 25. Risk Level Scale
**XGBoost output → clinical action**

- **Low (Score: 0.0 – 0.29)**: Standard passive monitoring. No JITAI unless engagement drops.
- **Moderate (Score: 0.30 – 0.59)**: Increased JITAI frequency. Gentle nudges. No clinician alert.
- **High (Score: 0.60 – 0.79)**: Clinician alert triggered. CBT/grounding interventions. Daily monitoring.
- **Crisis (Score: 0.80 – 1.00)**: Immediate escalation. Zero cooldown. NIMHANS helpline. Clinician instant alert.

---

## 26. Security, Privacy and Compliance

### DPDP Act 2023
- Explicit consent toggles on sign-up
- Purpose documented per data type
- GPS entropy only — never coordinates
- Right to delete all data
- Right to export all data

### Encryption and Security
- AES-256-GCM on all journal text
- Firebase JWT on every route
- Firestore rules: own data only
- `serviceAccountKey.json` gitignored
- Rate limiting via `express-rate-limit`

### Accessibility — WCAG 2.1 AA
- 44px minimum tap targets (72px elderly)
- `accessibilityLabel` on every element
- Risk always: color + icon + text
- Tamil text `accessibilityLanguage` declared
- Offline banner and graceful degradation

---

## 27. 9-Day Build Sprint (April 19–27, 2026)

| Day | Date | Backend + AI | Frontend + UI | ML Work |
| --- | --- | --- | --- | --- |
| 1 | Apr 19 | Auth routes, offline queue, Firestore indexes | Auth screens (signup/OTP/login) | — |
...
## 29. Smartwatch Integration Architecture (Google Health API)

Niranthara uses a Cloud-First Abstraction Layer to pull clinical-grade biometric data from Fitbit/WearOS devices via the Google Health API. This makes the platform device-agnostic, seamlessly feeding high-fidelity sensor telemetry directly into the XGBoost risk model.

### Service Folder Structure

The smartwatch integration operates as an independent top-level Node.js service running on port 5001.
...
├── mobile-app/
├── dashboard/
└── smartwatch/              ← new top-level service
    ├── index.js             # Express entry point (port 5001)
    ├── .env                 # Google OAuth credentials
    ├── package.json
    ├── config/
    │   └── firebase.js      # Shared Firebase Admin initialized via serviceAccountKey.json
    ├── routes/
    │   └── oauthRoutes.js   # Google OAuth 2.0 connect & callback
    ├── services/
    │   ├── BiometricSyncService.js   # Core cloud-to-cloud sync & XGBoost trigger
    │   └── biometricCron.js          # node-cron 15min sync schedule
    └── utils/
        └── healthApiClient.js        # Google Health API fetch utility
```

### 1. Google OAuth 2.0 Flow
- **`GET /smartwatch/auth/connect`**: Generates authorization URL with scopes: `heart_rate.read`, `activity.read`, `sleep.read`, `body_temperature.read`, `oxygen_saturation.read`, `respiratory_rate.read`.
- **`GET /smartwatch/auth/callback`**: Exchanges code for tokens. Tokens are **AES-256-GCM encrypted** and stored in the Firestore `users` collection under `googleHealthTokens: { accessToken, refreshToken, expiresAt }`. Redirects to mobile deep link `niranthara://smartwatch/connected`.

### 2. Biometric Feature Mapping

The `BiometricSyncService` fetches the following data points and maps them to XGBoost features using personal 30-day z-score baseline deviations:
...
| Sleep Quality | Fitbit sleep stages | `sleep` | `sleep_quality_score` |
| Active Minutes | Fitbit activity | `active-minutes` & `steps` | Augments `steps_deviation_score` |
| Basal Body Temp | Fitbit via Google Health | `body-temperature` | Augments `cycle_vulnerability_score` |
| Respiratory Rate | Fitbit sleep tracking | `respiratory-rate` | Augments `anxiety_level_avg_7d` |

### 3. ML Pipeline & Crisis Integration
After every successful 15-minute biometric sync:
- **Crisis Trigger**: If `hrv_deviation_score > 0.4` (HRV critically low) AND `rhr_deviation_score > 0.3` (RHR elevated), the backend calls `POST /api/crisis/detect`. If `crisisProbability > 0.6`, a real-time `clinicianAlert` is generated.
- **Dynamic Receptivity**: If `cardiac_stress_score` is elevated during a predicted high-vulnerability cycle window (from the LSTM model), JITAI receptivity polling frequency is increased.
- All derived deviation scores are passed to `POST /api/predict/risk` to augment the core 14-feature XGBoost fusion model.

### 4. Scheduled Synchronization
- **`biometricCron.js`**: Uses `node-cron` (`*/15 * * * *`) to sweep all users with active `googleHealthTokens`.
- Implements automatic token refresh via the Google token endpoint if `expiresAt < Date.now()`.
- Writes all telemetry to the `biometricLogs` Firestore collection, ensuring `offlineSyncId` is present to prevent data duplication.

### 5. Mobile App UI (SmartWatchCard)
- Displays Fitbit/WearOS connection status on the `Home.js` dashboard using Niranthara brand colors (Connected = Sage `#7BA68A`, Not Connected = Warm Gray `#8A8076`).
- Shows an SVG arc representing the current HRV deviation score, alongside the `lastBiometricSync` timestamp.

---

## 30. Comprehensive Feature List (End-to-End)
...
## 31. Developer Setup Checklist (Your Action Items)

Since this platform spans multiple services, local models, and cloud infrastructure, here is the exact checklist of items **you must complete on your machine/cloud accounts** to make the project fully operational.

### ✅ What is Already Done (In the Codebase)
- All UI, components, navigation, and styling.
- All machine learning models, training scripts, and NLP pipelines.
- All API routing, Firebase queries, and Cron job logic.
- All encryption algorithms and security middleware.
- **NVIDIA API Integration:** The chatbot is now powered by the NVIDIA Minimax API.
- The entire Google Health API OAuth implementation and fetch logic.

### 🛠️ What You Need to Do (Setup Required)

#### 1. Firebase Project Setup
1. Go to the [Firebase Console](https://console.firebase.google.com/).
2. Create a new project (e.g., `niranthara-app`).
3. Enable **Authentication** (Email/Password).
4. Enable **Firestore Database** (Create in `asia-south1`).
5. Set up **Firestore Rules** (allow read/write for testing).
6. Register a Web App to get your config for `dashboard/.env` and `mobile-app/src/utils/firebase.js`.
7. Register an Android App to enable Firebase Cloud Messaging (FCM).

#### 2. Firebase Admin SDK (Service Account)
1. In Firebase Console, go to **Project Settings > Service Accounts**.
2. Click **Generate New Private Key**.
3. Save the downloaded JSON file as exactly: `backend/serviceAccountKey.json`. *(The smartwatch service automatically reuses this file).*

#### 3. Google Health API (Smartwatch Credentials)
1. Go to the [Google Cloud Console](https://console.cloud.google.com/).
2. Create a project and enable the **Fitness API** and **Health Connect API**.
3. Go to **APIs & Services > Credentials** and create an **OAuth Client ID** (Web Application).
4. Add `http://localhost:5001/smartwatch/auth/callback` to the Authorized redirect URIs.
5. Set up the **OAuth Consent Screen** and add the scopes listed in `smartwatch/routes/oauthRoutes.js`.
6. Copy the Client ID and Client Secret into `smartwatch/.env`.

#### 4. Environment Variables & API Keys
1. **NVIDIA API Key:** Your real API key has already been added to `ai-service/.env`.
2. **Sarvam AI:** Get your key from [Sarvam.ai](https://www.sarvam.ai/) and add it to `backend/.env` and `ai-service/.env`.
3. **Encryption Key:** Generate a 64-character hex string (e.g., using `node -e "console.log(require('crypto').randomBytes(32).toString('hex'))"`) and add it to `backend/.env` and `smartwatch/.env`. **They must match exactly.**

#### 5. AI Service Dependencies & Local Models
1. **Chatbot:** No local setup needed! It uses the NVIDIA cloud API.
2. **NLP Models:** In `ai-service/`, create the virtual environment and run `python download_models.py` (Wait for the HuggingFace download to finish). These models handle Crisis and Sentiment detection locally.
